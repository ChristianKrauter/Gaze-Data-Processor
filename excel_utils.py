...
def fill_excel_col(dest_excel_fname, dest_col_name, dest_id_col_name, dest_sheet_name, source_excel_name, source_col_name, source_id_col_name, source_sheet_name, func_expected_data_type=None):
    # read the source excel into data_dict
    source_data = read_excel(
        source_excel_name, source_id_col_name, source_sheet_name)
    ...
    edited_destinated_excel = xlsxwriter.Workbook(
        os.path.join(edited_excel_dir, edited_excel_fname))
    edited_destinated_sheet = edited_destinated_excel.add_worksheet(
        dest_sheet_name)
    ...
    for row_index in xrange(0, destinated_sheet.nrows):
        ...
        destinated_id = str(destinated_sheet.cell(
            row_index, destinated_id_col_index).value)
        # skip empty row
        ...
        if destinated_id in source_data:
            ...
            edited_destinated_sheet.write(
                row_index, destinated_col_index, source_value)

    # save the excel
    edited_destinated_excel.close()


def fill_excel_cols(dest_excel_fname, dest_col_names, dest_id_col_name, dest_sheet_name, source_excel_name, source_col_names, source_id_col_name, source_sheet_name, func_expected_data_type=None):
    for i, dest_col_name in enumerate(dest_col_names):
        source_col_name = source_col_names[i]
        fill_excel_col(dest_excel_fname, dest_col_name, dest_id_col_name, dest_sheet_name, source_excel_name,
                       source_col_name, source_id_col_name, source_sheet_name, func_expected_data_type)
...
def fill_score_data():
    sour_excel_fname = '/Users/lguan/Documents/Study/Research/Gaze-Dataset/data_processing/csv/stat_data.xlsx'
    sour_id_name = 'trial_id'
