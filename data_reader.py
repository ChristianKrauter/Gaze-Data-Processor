...
def read_gaze_data_asc_file(fname):
    ...
    file_meta_data = {'avg_error': None, 'max_error': None,
                      'low_sample_rate': None, 'total_frame': None}
    ...
    for (i, line) in enumerate(lines):
        ...
        if match_validation:
            avg_error, max_error = match_validation.group(
                2), match_validation.group(3)
    ...
    file_meta_data['low_sample_rate'] = "{:.1f}".format(
        100.0*float(few_cnt)/float(n_frame)) + "%"
